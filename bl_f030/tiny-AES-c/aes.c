...
void AES_CBC_init(const uint8_t* key, const uint8_t* iv) {
  ...
  */
  Key = key;
  KeyExpansion();
  memcpy(Iv, iv, BLOCKLEN);
}

void AES_CBC_decrypt_buffer(uint8_t* buf, uint32_t length) {
  assert(length % BLOCKLEN == 0);

  static uint8_t last[BLOCKLEN];
  for (uint32_t i = 0; i < length; i += BLOCKLEN)
  {
    memcpy(last, buf, BLOCKLEN);
    state = (state_t*)buf;
    InvCipher();
    XorWithIv(buf);
    memcpy(Iv, last, BLOCKLEN);
    buf += BLOCKLEN;
  }
}
